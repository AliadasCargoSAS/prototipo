...
$query = "SELECT IdPrincipal, NumeroFicha, Programa, Competencia, Rap, NumeroAmbiente, Dia,substring(HoraInicio, 1, 5), substring(HoraFin, 1, 5), hour(HoraInicio), hour(HoraFin) FROM eventoinstructor WHERE CabeceraInstructor = '$instructor' and Competencia <> '' and Rap <> '' and NumeroAmbiente <> ''";
$resultado = $link->query($query);

$query2 = "SELECT fechaInicio from eventoinstructor where CabeceraInstructor='$instructor' limit 1";
$resultado2 = $link->query($query2);

$query3 = "SELECT fechaFin from eventoinstructor where CabeceraInstructor='$instructor' limit 1";
...
$pdf->SetFillColor(237, 255, 237);
    ...
    $pdf->Image('https://imgur.com/6R5EFuH.png',23,21,14);
...
$pdf->Cell(103, 7, "   INSTRUCTOR:  $instructor", 1, 0, 'L', 1);
if ($result = mysqli_query($link, $sql2)) {
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_array($result)) {
            $tmtreaño = $row['Trimestre año'];
            $pdf->Cell(55, 7, "   TRIMESTRE DEL AÑO:  $tmtreaño", 1, 0, 'L', 1);
        }
    }
}
$año = date('Y');
$pdf->Cell(40, 7, "   AÑO:  $año", 1, 0, 'L', 1);
while ($row = mysqli_fetch_array($resultado2)) {
    $inicio = $row["fechaInicio"];
    $pdf->Cell(55, 7, "   FECHA INICIO:  $inicio", 1, 0, 'L', 1);
}
while ($row = mysqli_fetch_array($resultado3)) {
    $fin = $row["fechaFin"];
    $pdf->Cell(55, 7, "   FECHA FIN:  $fin", 1, 0, 'L', 1);
}
$pdf->Ln(2);
$pdf->WriteHTML("$htmlTablehead . $htmlTablebody . $htmlTablefooter");
$pdf->Output('D', 'Evento Instructor '.$instructor.'.pdf');
