...
if($_SERVER["REQUEST_METHOD"] == "POST"){
   ...
   if(empty($input_tipocontrato)){
    ...
    if(empty($identificacion_err) && empty($nombre_err) && empty($nacimiento_err) && empty($telefono_err) && empty($especialidad_err) && empty($correo_err) && empty($TipoContrato_err) && empty($contratoI_err) && empty($contratoF_err)){
        ...
        $sql = "INSERT INTO instructores (Identificacion, Nombre, Nacimiento, Telefono, Especialidad, Correo, TipoContrato, ContratoInicio, ContratoFin, Informacion, Status) VALUES (?, upper(?), ?, ?, upper(?), upper(?), upper(?), upper(?), upper(?), upper(?), '1')";
         
        if($stmt = mysqli_prepare($link, $sql)){
            
            mysqli_stmt_bind_param($stmt, "isssssssss", $param_identificacion, $param_nombre, $param_nacimiento, $param_telefono, $param_especialidad, $param_correo, $param_TipoContrato, $param_contratoI, $param_contratoF, $param_informacion);
    ...
    <div class="container mb-3">
        <div class="panel" id="cont-bloque-corto2">
            ...
            <div class="form-group <?php echo (!empty($TipoContrato_err)) ? 'has-error' : ''; ?>">
                ...
                <select name="TipoContrato" style="font-size:16px;color:#495057;text-transform: uppercase;" class="form-control" value="<?php echo $TipoContrato; ?>">                                                
                        <option> </option>
